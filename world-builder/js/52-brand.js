...
(function (WB) {
  ...
  B.untouched = function () {
    const b = S.P.brand;
    if (S.BRAND_META_FIELDS.some((d) => metaMN(d.k))) return false;
  ...
  B.masterPrompt = function () {
    ...
    const topics = metaEN("topics")
      .split(/\r?\n/)
      .map((s) => s.replace(/^\s*(?:[-*•]|\d+[.)])\s*/, "").trim())
      .filter(Boolean);
    if (topics.length) {
      L.push("Topic queue:");
      topics.forEach((t) => L.push("  - " + t));
    }
  ...
  B.render = function () {
    ...
    /* Судалгааны талбарууд — ниш, формат дээд хайрцагт; сэдвийн жагсаалт
       «5 сэдэв санал болгох» товчны доор өөрийн хайрцагт */
    const boxes = {};
    S.BRAND_META_FIELDS.forEach((d) => {
      const id = d.box || "brandMetaFields";
      if (!(id in boxes)) {
        boxes[id] = el(id);
        if (boxes[id]) boxes[id].innerHTML = "";
      }
      if (boxes[id]) boxes[id].appendChild(WB.ui.dualField(b.meta[d.k], d.kind || "brand", d.lb, d.ph));
    });
