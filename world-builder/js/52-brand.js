...
(function (WB) {
  ...
  B.masterPrompt = function () {
    ...
    L.push("", "=== LAYER 2 · LOOK" + (B.isLocked() ? " (locked on one frame)" : "") + " ===");
  ...
  const RX = {
    ...
    camera: /\b(takes?|cuts?|shots?|tracking|tracks?|dolly|pans?|tilts?|crane|handheld|steadicam|gimbal|lens|\d+\s?mm|close-?ups?|wide|framing|camera|angles?|zoom|push-?in|pull-?out|orbit(ing)?|aerial|pov|depth of field|bokeh|slow motion|slow-?mo|timelapse|motion blur|shake|shaky|whip pan|rack focus)\b/i,
    flare: /\b(lens flares?|flares?|glare)\b/i,
    ...
    /* Уур амьсгалын тэмдэг нэр — богино хэллэг бол хэв маяг */
    mood: /^(?:(?:very|slow|fast|quiet|violent|sudden|overwhelming|calm|tense|serene|chaotic|intense|melancholic|dramatic|eerie|peaceful|ominous|heavy|somber|sombre|hopeful|nostalgic|lonely|warm|cold|dark|bleak|tender|gentle|brutal|epic|mysterious|haunting)(?:,\s*|\s+(?:and\s+)?)?)+$/i,
    /* Өнгөний мөр: өнгөний үг + орчин/өнгөний нэр үг («Deep black space»),
       харин «white-grey EVA spacesuit» мэт объект биш */
    paletteNoun: /\b(palette|tones?|colou?rs?|hues?|haze|mist|fog|smoke|space|sky|light|glow|grade|grading|shadows?|bands|tint|wash|backdrop|background|atmosphere|night|dusk|dawn|water|sea|desert|steppe|horizon)\b/i,
    /* Үйл явдал: «the pod is hurled», «then …», «suddenly …» */
    action: /\b(is|are|was|were|gets?|being)\b|^(then|suddenly|while|as|and then)\b/i
  };
  ...
  B.splitPrompt = function (text) {
    ...
    /* Мөр дундуураа таслагдсан (hard-wrap) промтыг буцааж нийлүүлнэ:
       хоосон мөр = шинэ догол; цагийн хуваарь, [@лавлагаа], жагсаалтын
       тэмдэгээр эхэлсэн мөр = шинэ хэсэг; бусад мөр өмнөхдөө залгагдана. */
    const segs = [];
    String(text || "")
      .split(/\r?\n/)
      .forEach((raw) => {
        const line = normTxt(raw);
        if (!line) return segs.push(null);
        const fresh = TIMELINE.test(line) || /^[\[@]/.test(line) || /^([-*•]|\d+[.)])\s/.test(line);
        const last = segs.length ? segs[segs.length - 1] : null;
        if (last === null || fresh) segs.push(line);
        else segs[segs.length - 1] = last + " " + line;
      });
    segs.filter(Boolean).forEach((seg) => {
      if (TIMELINE.test(seg)) return pacing.push(seg);
      seg.split(/(?<=[.!?])\s+/).forEach((sen) => {
        sen = sen.replace(/[.!?]+$/, "").trim();
        if (!sen) return;
        if (REFERENCE.test(sen)) return ref.push(sen + ".");
        ...
        parts.forEach((c, idx) => {
          ...
          else if (RX.flare.test(t)) k = "light";
          else if (RX.camera.test(t)) k = "camera";
          else if (RX.light.test(t)) k = "light";
          else if (RX.palette.test(t) && RX.paletteNoun.test(t)) k = "palette";
          else if (RX.lookWeak.test(t) || (nw(t) <= 5 && RX.mood.test(t))) k = "look";
          ...
          /* «then the pod is hurled past the lens» — үйл явдал, брэнд биш */
          if (!neg && RX.action.test(t) && !(k === "light" && LIGHT_STRONG.test(t))) return;
        ...
        /* Эхний дүрслэлийн өгүүлбэр — туршилтын кадр (бүтнээр, үгчлэн) */
        if (!frame && !firstUsed && nw(sen) >= 6) frame = sen;
        else if (!anyUsed) skipped.push(sen);
      });
    });
    return { rows: rows, pacing: pacing, frame: frame, ref: ref, skipped: skipped };
  };
  ...
  B.decompose = async function (text) {
    ...
    S.pushHistory();
    /* Өмнөх промтоос орсон мөрүүдийг цэвэрлэнэ — шинэ промтод байхгүй
       бол хуучин промтын үлдэгдэл холилдохгүй. Таны өөрөө бичсэнд хүрэхгүй. */
    const clear = (f) => {
      if (f && f.src === "prompt") {
        f.en = "";
        f.mn = "";
    ...
    Object.keys(b.f).forEach((k) => clear(b.f[k]));
    clear(b.frameSubject);
    if (b.direction) S.DIRECTION_FIELDS.forEach((x) => clear(b.direction[x.k]));
    if (b.refSrc === "prompt") {
      b.ref = "";
      b.refSrc = "";
    }
    ...
    S.touch();
    return { rows: out.sort((a, c) => a.layer - c.layer), untouched: untouched, skipped: sp.skipped, ai: tr.ai };
  };
    ...
    if (rep) {
      ...
      rep.innerHTML =
        ...
        lastReport.rows
          ...
          .join("") +
        "</table>" +
        (lastReport.skipped && lastReport.skipped.length
          ? '<p class="note">Брэндэд <b>ороогүй</b> — нэг ангийн үйл явдал, дүрийн заавар (англи промтод хэвээр):<br>' +
            lastReport.skipped.map((t) => "• " + U.esc(t.length > 110 ? t.slice(0, 110) + "…" : t)).join("<br>") +
            "</p>"
          : "") +
