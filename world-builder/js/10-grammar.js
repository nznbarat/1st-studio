...
(function (WB) {
  ...
  G.POSTP = {
    ...
    /* харьцуулалт: «тоос шиг» → «like dust», «хөшөө мэт» → «like a monument» */
    "шиг": "like", "мэт": "like"
  };

  /* ── тоо ба нэгж: «35мм» → 35mm, «1-р» → 1st, «8с» → 8s ───── */
  G.ordinal = function (n) {
    const k = n % 100;
    if (k >= 11 && k <= 13) return n + "th";
    return n + ({ 1: "st", 2: "nd", 3: "rd" }[n % 10] || "th");
  };
  G.numeric = function (w) {
    let m = /^(\d+(?:[.,]\d+)?)мм$/.exec(w);
    if (m) return m[1].replace(",", ".") + "mm";
    m = /^(\d+)-?р$/.exec(w);
    if (m) return G.ordinal(+m[1]);
    m = /^(\d+(?:[.,]\d+)?)с$/.exec(w);
  ...
  const SUF = [
    ...
    /* ь‑ээр төгссөн үг: морь → морины, хонь → хонины, морь → морьд.
       Гурав дахь элемент нь хассаны дараа үндэст буцааж залгах үсэг. */
    ["ины", ["gen"], "ь"], ["ьд", ["pl"], "ь"],
    ...
    ["ж", ["v"]], ["ч", ["v"]],
    /* далд «н»‑тэй өгөх орших: нүүр → нүүрэнд, шил → шилэнд */
    ["анд", ["dat"]], ["энд", ["dat"]], ["онд", ["dat"]], ["өнд", ["dat"]],
    /* гийгүүлэгчийн араас: секунд → секундэд */
    ["ад", ["dat"]], ["эд", ["dat"]], ["од", ["dat"]], ["өд", ["dat"]],
    ["д", ["dat"]], ["т", ["dat"]],
    ["ий", ["gen"]],
    ["н", []],
    /* материалын тэмдэг нэр: цас → цасан, мод → модон. «н»‑ээс хойш байх ёстой —
       эс бөгөөс нөхцөл үйл үг «харан» (харж) нь «хар» (black) болж таарна. */
    ["ан", []], ["эн", []], ["он", []], ["өн", []],
    ["г", ["acc"]]
  ];
  ...
  G.analyze = function (word) {
    ...
    for (let depth = 0; depth < 2; depth++) {
      ...
      for (const cur of layer) {
        for (const [suf, tags, restore] of SUF) {
          if (cur.stem.length > suf.length + 1 && cur.stem.endsWith(suf)) {
            const base = cur.stem.slice(0, cur.stem.length - suf.length) + (restore || "");
